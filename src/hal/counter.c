...
{
    bool ok = device_is_ready(MORT_DT_DEV_IR_COUNTER);
    MORT_RETURN_LOGE_IF(!ok, -EIO, "IR counter device is not ready");

    uint32_t mtv = counter_get_max_top_value(MORT_DT_DEV_IR_COUNTER);
    MORT_LOGD("Max top value of the IR counter: %u", mtv);

#if (0)
    struct counter_top_cfg top_cfg = {.ticks = 0xFFFF};
    int                    ec      = counter_set_top_value(MORT_DT_DEV_IR_COUNTER, &top_cfg);
...
{
    int ec = counter_start(MORT_DT_DEV_IR_COUNTER);
    MORT_RETURN_LOGE_IF(ec, -EIO, "Failed to start the IR counter");

    struct counter_top_cfg top_cfg = {.ticks = 0xFFFF};
    ec                             = counter_set_top_value(MORT_DT_DEV_IR_COUNTER, &top_cfg);
    MORT_RETURN_LOGE_IF(ec, -EIO, "Failed to set the top value of the IR counter (ec: %d)", ec);
...
{
    int ec = counter_stop(MORT_DT_DEV_IR_COUNTER);
...
{
    int ec = counter_get_value(MORT_DT_DEV_IR_COUNTER, o_count);
